...
class Jarvis:
    def __init__(self, args):
        ...
        self.logger.info("Initiating db_connection with : {}".format(self.db_path))
        try:
            ...
            self.logger.exception("Error initiating database: {}".format(e))
            sys.exit(1)
    ...
    def run_modules(self):
        try:
            for module, attrs in self.modules.items():
                ...
                attrs['logging'] = self.config.logging
                mod_func = globals()[module]
                try:
                    self.logger.info("Launching {}".format(module))
                    t = mod_func(attrs)
                    t.start()
                    self.threads.append(t)
                except Exception as e:
                    self.logger.exception("Error launching module {}: {}".format(module, e))
            while True:
                m = self.message_q.get()
                if type(m) == TextMessage:
                    src = m.src
                    self.logger.info("Getting contact for {}".format(src))
                    any_contact = self.session.query(Contact).first()
                    if not any_contact:
                        contact = utils.create_contact(self.session, src)
                        user = utils.create_user(self.session, 'admin', contact)
                    else:
                        contact = self.session.query(Contact).filter(Contact.number == m.src).first()
                    if not contact:
                        self.logger.info("No contact for {}, not sending to modules".format(m))
                        continue
                    else:
                        contact.increment_incoming()
                    user = self.session.query(User).get(contact.user_id)
                    if not user:
                        self.logger.info("Pass to introduction module?")
                        continue
                    if user.has_permissions(m.body):
                        self.logger.info("Contact {} has permissions to run {}".format(contact, m.body))
                    else:
                        self.logger.info("Contact {} does not have permissions to run {}".format(contact, m.body))
                        continue
                ...
                self.logger.info("Distributing message to consumers: {}".format(m))
            ...
            self.logger.info("Ctrl+C, RIP threads.")
            self.Session.remove()
            self.cull_threads()
        except Exception as e:
            self.logger.exception("Core Exception: {}".format(e))
            self.Session.remove()
            self.cull_threads()
